 ...
 *              1、i + j = m + n - i - j (当m+n为奇数时:m + n - i -j + 1) 表示左右两边数字数量相等。
 ...
 *          当算法解得的i不是0或者m时，则代表四个边界均存在，因此要满足:A[i-1] < B[j] && B[j-1] < A[i]。
 *          当算法解得的i为下界0时，则有A[i-1]不存在，因此只需要比较:B[j-1] < A[i]。
 *          当算法解得的i为上界m是，则有A[i]不存在，因此只需要比较：A[i-1] < B[j]。
